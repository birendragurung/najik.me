...
<div class="col-md-12">
    @if(count($businesses)>0)
        @foreach($businesses as $business)
            <div class="col-md-4 mb-5 business-wrapper" business-id={{$business->id}} lat="{{$business->address->latitude }}" long="{{$business->address->longitude }}">
                <div class="card-deck">
                    <div class="card">
                        <a href="{{url('/business/'.$business->id)}}">
                            ...
                            <div class="card-body col-xs-12 col-sm-12 col-md-12">
                                ...
                                <h6><a href="/categories/{{$business->category->id}}">{{$business->categoryName }}</a>
                                </h6>
                            ...
                            <div class="row">
                                ...
                                @if(Auth::user() && Auth::id() != $business->user->id)
                                    <form class="col-sm-8 col-md-8 col-lg-9 mb-3 rating-form">
                                        <input id="input-{{$business->id}}" value="{{$business->myRating}}" type="text" class="rating rating-input" data-min=0 data-max=5 data-step=1 data-size="xs" data-show-catption=false required title="">
                                    </form>
